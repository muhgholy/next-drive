...
export const DrivePathBar = ({ className }: { className?: string }) => {
    ...
    return (
        <ol className={cn("nd:flex nd:items-center nd:gap-1 nd:sm:gap-1.5 nd:text-sm nd:text-muted-foreground nd:overflow-x-auto nd:flex-nowrap nd:min-w-0", className)} aria-label="Breadcrumb" role="navigation">
            {path.map((item, index) => {
                const isLast = index === path.length - 1;
                return (
                    <li key={item.id ?? 'root'} className="nd:flex nd:items-center nd:gap-1 nd:sm:gap-1.5 nd:shrink-0">
