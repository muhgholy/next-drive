...
import {
    ...
    Database, HardDrive, ChevronsUpDown, Check, Plus, Settings2, Pencil, Trash
} from 'lucide-react';

// ============================================================================
// INLINE SIDEBAR - Simplified for the chooser context
// ============================================================================
const ChooserSidebar = () => {
    const {
    ...
    const openOAuthPopup = async () => {
        ...
        if (res.status !== 200 || !res.data?.url) { alert(res.message || 'Failed'); return; }
        const width = 600, height = 600;
        const left = window.screen.width / 2 - width / 2;
        const top = window.screen.height / 2 - height / 2;
        window.open(res.data.url, 'Connect to Google Drive', `width=${width},height=${height},top=${top},left=${left}`);

        const cleanup = () => { window.removeEventListener('message', ml); window.removeEventListener('storage', sl); };
        const ml = (e: MessageEvent) => { if (e.data === 'oauth-success') { cleanup(); refreshAccounts(); } };
        const sl = (e: StorageEvent) => { if (e.key === 'next-drive-oauth-success') { cleanup(); refreshAccounts(); } };
            ...
            <div className="p-2 border-b border-border/50">
                <DropdownMenu>
                    ...
                    <DropdownMenuContent className="w-56" align="start">
                        ...
                        <DropdownMenuItem onClick={() => setActiveAccountId(null)} className="gap-2 py-2">
                        ...
                        {accounts.map(account => (
                            <div key={account.id} className="flex items-center group">
                                <DropdownMenuItem onClick={() => setActiveAccountId(account.id)} className="flex-1 gap-2 py-2 pr-1">
                                ...
                                <DropdownMenu>
                                    <DropdownMenuTrigger asChild>
                                        <Button variant="ghost" size="icon" className="size-7 mr-1 opacity-0 group-hover:opacity-100 transition-opacity" onClick={(e) => e.stopPropagation()}>
                                            <Settings2 className="size-3.5 text-muted-foreground" />
                                        </Button>
                                    </DropdownMenuTrigger>
                                    <DropdownMenuContent align="end" className="w-36">
                                        <DropdownMenuItem onClick={() => { setNewName(account.name); setRenameDialog({ open: true, account }); }}>
                                            <Pencil className="size-3.5 mr-2" /> Rename
                                        </DropdownMenuItem>
                                        <DropdownMenuSeparator />
                                        <DropdownMenuItem className="text-destructive focus:text-destructive" onClick={() => setDeleteDialog({ open: true, account })}>
                                            <Trash className="size-3.5 mr-2" /> Remove
                                        </DropdownMenuItem>
                                    </DropdownMenuContent>
                                </DropdownMenu>
                            </div>
            ...
            <Dialog2 open={isOpen} onOpenChange={setIsOpen}>
                <Dialog2Content showCloseButton={false}>
                    ...
                    <Dialog2Body className="flex flex-col md:flex-row">
                        ...
                        {/* Main Content - Wrapped in DnD Provider for path bar + grid drag */}
                        <DriveDndProvider>
                            <div className="flex-1 flex flex-col min-w-0 min-h-0">
                                ...
                                {/* Path + Upload Row */}
                                <div className="shrink-0 flex items-center gap-2 px-3 py-2 border-b bg-muted/20 dark:bg-muted/10">
                                    <div className="flex-1 min-w-0">
                                        <DrivePathBar />
                                    </div>
