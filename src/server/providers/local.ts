...
import { extractImageMetadata, computeFileHash, moveFile } from '@/server/utils';
...
export const LocalStorageProvider: TStorageProvider = {
    ...
    move: async (id, newParentId, owner, accountId) => {
        ...
        // For LOCAL, files are stored in a flat structure by ID.
        // Path is relative to storage.path: <ID>/data<ext>
        // So moving a file DOES NOT change its physical path on disk.
        // Hierarchy is virtual in DB. Physical storage is flat (by ID).
